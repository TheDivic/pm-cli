---
name: kb-sync
description: Mirror progress, decisions, and spec changes from this repository into the Plaintext Brain knowledge base at ~/plaintext-brain, which is the source of truth for the plaintext-tasks project. Use after completing or starting a piece of work, after making a decision, when CLI behavior changes, or when committing to the knowledge base repo.
---
...
Location: `/Users/divic/plaintext-brain/projects/plaintext-tasks/`

| file | holds | how to change it |
|------|-------|------------------|
| `plaintext-tasks.tasks.yaml` | task state and progress | **`pm` only** — never hand-edit |
| `plaintext-tasks.md` | decisions (Decisions section), open questions, resources | edit directly |
| `plaintext-tasks.notes.md` | live, provisional, or unresolved observations — *not* task state | edit directly |
...
The task file is a `*.tasks.yaml` file, so the `plaintext-tasks` skill's one rule
...
pm tasks add -p plaintext-tasks -t "<what the work is>"
...
- A real decision goes in the Decisions section of `plaintext-tasks.md`, as one
...
- **If a change alters documented CLI behavior, the frozen
  ...
  frozen spec.
- Provisional or unresolved things go in `plaintext-tasks.notes.md`, not into task
...
- **No conventional-commit type or scope prefixes** — that style is tasks-cli only.
...
`plaintext-tasks.tasks.yaml`, say), it is fine to carry theirs along — but note it
