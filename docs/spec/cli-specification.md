...
`projects list` supports `-a`/`--all`, `--status`, `--priority`, and `--area`. By default it shows every project except `cancelled` (`backlog`, `ready`, `in-progress`, `in-review`, `blocked`, `done`) — cancelled is the one status excluded, since it is work that will never be finished and, unlike done, has no value as a visible record. The `-a`/`--all` flag includes `cancelled` projects too, and an explicit `--status` filter overrides the default — the same rule `tasks list` follows.

Human-readable output starts with a header row naming the columns (`ID`, `TITLE`, `CREATED`, `PROGRESS`), then groups projects by status into labeled sections below it, read top to bottom as the lifecycle pipeline: `backlog`, `ready`, `in-progress`, `blocked`, `in-review`, `done`, then (with `-a`) `cancelled`. `blocked` is slotted right after `in-progress`, where a project actually stalls, rather than ranked by closeness to completion. Each section header names the status in full capitals and the count of projects in it (for example `IN-PROGRESS · 3`), styled bold rather than set off with a rule so it still reads as a label and not a data row without extra visual weight; a section with no matching projects is omitted entirely, and a result with nothing to show at all reports that no projects matched instead of printing nothing. Within a section, projects sort by priority (lowest number first, unprioritized last), then creation date (oldest first), then project ID. Each row lists the project ID, title, creation date, and a compact completion progress bar; status is not repeated as a column since the section header already states it, and priority is not shown since it already governs the sort order — `projects show` reports both. The ID and title columns share one width across every section and the header row, so progress bars line up regardless of which section they fall in.

The `inbox` project (see `tasks add`) is excluded from `projects list` entirely: it is a project, not a status, and its perpetual, always-open nature does not fit a status-grouped or status-filtered view. Use `pm tasks list --project inbox` to see what is waiting in it.

In JSON mode, projects are not grouped: they list flat, in-review first, then in-progress, then all other statuses, sorted the same way within each group, with per-status task counts, and the inbox project included like any other project — the human-readable exclusion is a display choice, not a data-model one. The task-file path is not shown in either mode; `projects show` reports it in project details.
...
`projects create` accepts optional `--path`, `--status`, `--priority`, `--due`, and repeated `--area` flags. Its default status is `backlog`, and its default path is `<root>/<project-id>/<project-id>.tasks.yaml`. It creates the target directory when needed but refuses to replace an existing task file.
