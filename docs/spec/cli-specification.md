...
Human-readable output groups projects by status into labeled sections, read top to bottom as the lifecycle pipeline: `idea`, `todo`, `in-progress`, `blocked`, `in-review`, then (with `-a`) `done` and `cancelled`. `blocked` is slotted right after `in-progress`, where a project actually stalls, rather than ranked by closeness to completion. Each section header names the status in full capitals and the count of projects in it (for example `IN-PROGRESS · 3`); a section with no matching projects is omitted entirely, and a result with nothing to show at all reports that no projects matched instead of printing nothing. Within a section, projects sort by priority (lowest number first, unprioritized last), then creation date (oldest first), then project ID. Each row lists the project ID, title, creation date, and a compact completion progress bar; status is not repeated as a column since the section header already states it, and priority is not shown since it already governs the sort order — `projects show` reports both. The ID and title columns share one width across every section so progress bars line up regardless of which section they fall in.

The `inbox` project (see `tasks add`) is pinned above every section as a single `INBOX · <count>` line reporting its total task count, rather than appearing in a status section with a status or progress bar — neither is meaningful for a perpetual capture bucket.

In JSON mode, projects are not grouped: they list flat, in-review first, then in-progress, then all other statuses, sorted the same way within each group, with per-status task counts, and the inbox project included like any other. The task-file path is not shown in either mode; `projects show` reports it in project details.
